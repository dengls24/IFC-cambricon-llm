...
IfcSimulationRow ifc_simulate_one_with_system(
    ...
    row.cycle_issue_width = cycle_weight_ok == 0 ? cycle_stats.issue_width : 0;
    row.cycle_issue_fifo_depth = cycle_weight_ok == 0 ? cycle_stats.issue_fifo_depth : 0;
    row.cycle_module_clock_ns = cycle_weight_ok == 0 ? cycle_stats.module_clock_ns : 0.0;
    row.cycle_stage_issue_events = cycle_weight_ok == 0 ? cycle_stats.stage_issue_events : 0;
    row.cycle_dispatch_rounds = cycle_weight_ok == 0 ? cycle_stats.dispatch_rounds : 0;
...
static int write_csv(const char *path, const IfcSimulationRow rows[IFC_ROW_COUNT]) {
    ...
    fprintf(file,
            ...
            "cycle_issue_width,cycle_issue_fifo_depth,cycle_module_clock_ns,cycle_stage_issue_events,"
            "cycle_dispatch_rounds,"
    ...
    for (int i = 0; i < IFC_ROW_COUNT; ++i) {
        ...
        fprintf(file,
                ...
                r->cycle_issue_width,
                r->cycle_issue_fifo_depth,
                r->cycle_module_clock_ns,
                r->cycle_stage_issue_events,
                r->cycle_dispatch_rounds,
...
static int write_json(const char *path, const IfcSimulationRow rows[IFC_ROW_COUNT], const IfcSummary *summary) {
    ...
    for (int i = 0; i < IFC_ROW_COUNT; ++i) {
        ...
        if (rows[i].cycle_stage_issue_events > max_cycle_stage_issue_events) {
            max_cycle_stage_issue_events = rows[i].cycle_stage_issue_events;
        }
    ...
    fprintf(file, "    \"max_cycle_stage_issue_events\": %lld,\n", max_cycle_stage_issue_events);
...
static int write_latency_breakdown(const char *path, const IfcSimulationRow rows[IFC_ROW_COUNT]) {
    ...
    for (int i = 0; i < IFC_ROW_COUNT; ++i) {
        ...
        fprintf(file,
                ...
                "full-row microcycle-derived flash weight stage after platform efficiency\n",
                r->model,
                r->platform,
                r->controller_weight_stage_ms,
                r->controller_weight_stage_ms);
...
static int write_cycle_weight_timing(const char *path, const IfcSimulationRow rows[IFC_ROW_COUNT]) {
    ...
    fprintf(file,
            ...
            "read_slice_data_cycles,array_read_cycles,ifc_compute_cycles,issue_width,issue_fifo_depth,"
            "module_clock_ns,stage_issue_events,dispatch_rounds,tpot_ms\n");
    for (int i = 0; i < IFC_ROW_COUNT; ++i) {
        const IfcSimulationRow *r = &rows[i];
        fprintf(file,
                "%s,%s,%.6f,%.6f,%.6f,%d,%lld,%lld,%lld,%lld,%.6f,%.6f,"
                "%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%d,%d,%.6f,%lld,%lld,%.6f\n",
                r->model,
                r->platform,
                r->cycle_ns,
                r->read_compute_requests,
...
static int write_report(const char *path, const IfcConfig *config, const IfcSimulationRow rows[IFC_ROW_COUNT], const IfcSummary *summary) {
    ...
    fprintf(file, "- `ssdsim_ifc_event_trace.csv` records ISSUE/COMPLETE events from the SSDsim-derived event loop.\n");
    fprintf(file, "- `ssdsim_ifc_event_stats.csv` records event-loop completion and resource-concurrency statistics.\n");
    ...
    fprintf(file, "- The C plot helper may emit local plot-source CSV files under this output directory's `figures/` folder for test inspection; those files are not release artifacts.\n");
    fprintf(file, "\n## Sanity Checks\n\n");
...
int ifc_write_outputs_config(const char *output_dir, const IfcConfig *config, const IfcSimulationRow rows[IFC_ROW_COUNT], const IfcSummary *summary) {
    ...
    char ssdsim_ifc_event_trace_path[4096];
    char ssdsim_ifc_event_stats_path[4096];
    ...
    join_path(ssdsim_ifc_event_trace_path, sizeof(ssdsim_ifc_event_trace_path), output_dir, "ssdsim_ifc_event_trace.csv");
    join_path(ssdsim_ifc_event_stats_path, sizeof(ssdsim_ifc_event_stats_path), output_dir, "ssdsim_ifc_event_stats.csv");
    ...
    if (ifc_write_ssdsim_ifc_event_trace_for_platform(
            ssdsim_ifc_event_trace_path,
            ssdsim_ifc_event_stats_path,
            &config->platforms[0]) != 0) {
        return -1;
    }
